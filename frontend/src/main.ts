...
type UsageOverview = {
  ...
  model_input_tokens: Record<string, number>;
  model_output_tokens: Record<string, number>;
};
...
function estimateSpend(overview: UsageOverview): number {
  return Object.entries(overview.models).reduce((sum, [model, outputTokens]) => {
    ...
    return sum + (inputTokens / 1_000_000) * pricing.input + (billedOutput / 1_000_000) * pricing.output;
  }, 0);
}
